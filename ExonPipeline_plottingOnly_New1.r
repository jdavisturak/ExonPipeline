...
save.image("PlottedData_new2.RData")
...
boxplot(log10(abs(start-end)) ~ exonLocationGroups, exons2, notch=T, yaxt='n',names=c('First','Internal','Last'), main='',ylab=sprintf("Exon lengths in\n%s genome",settings$CommonName))
...
for(column in c('Stability','Acceptor')){
  ...
  pdf(sprintf('%s_%s_new1.pdf',settings$CommonName,column),height=4,width=4*5)  
  ...
  plot(log10(Qdata[,1]), Qdata[,2],xaxt='n',pch=pchLast[1],xlim=log10(c(80,420000)),ylim=c(-1.24,1.24),main=colText, ylab=paste('Median',colText), xlab="Distance to End of Gene") 
...
HK = list(0,1,0:1)
  ...
  pdf(sprintf('%s_Stability_by_numIntrons_geneSize%s2.pdf',settings$CommonName,txt),height=3,width=4)
  par(mai=c(.45,.45,0.24,0.1),cex=0.6, cex.main=1) 
  
  for(i in 0:1){
    boxplot(Stability ~ intronCountGroups+GeneSize,data=subset(mergedData,isHK%in%h),notch=T,staplewex=0,ylim=c(-1.25,1.25),col=cols[1:5],at=c(1:5,7:11,13:17,19:23),outline=F,range=0.00001,add=i,ylab='Nucleosome Stability score',xaxt='n')
  ...
  pdf(sprintf('%s_Acceptor_by_numIntrons_geneSize%s2.pdf',settings$CommonName,txt),height=3,width=4)
  par(mai=c(.45,.45,0.24,0.1),cex=0.6, cex.main=1)
  for(i in 0:1){
...
HK = list(0,1)
  ...
  #png(sprintf('%s_Stability_by_numIntrons_geneSize.png',settings$CommonName),height=300,width=400)
  pdf(sprintf('%s_Stability_by_numIntrons_geneSize%s.pdf',settings$CommonName,txt),height=3,width=4)
  par(mai=c(.45,.45,0.24,0.1),cex=0.6, cex.main=1) 
  ...
  #png(sprintf('%s_Acceptor_by_numIntrons_geneSize.png',settings$CommonName),height=300,width=400)
  pdf(sprintf('%s_Acceptor_by_numIntrons_geneSize%s.pdf',settings$CommonName,txt),height=3,width=4)
  par(mai=c(.45,.45,0.24,0.1),cex=0.6, cex.main=1)
  for(i in 0:1){
...
######################################################
pdf(sprintf('%s_isLast_boxplots.pdf',settings$CommonName),height=6,width=2)
...
refseq$polyAstart = refseq$txEnd - UP
refseq$polyAend = refseq$txEnd + DOWN

refseq$polyAstart[refseq$strand=='-'] = refseq$txStart[refseq$strand=='-'] - UP
refseq$polyAend[refseq$strand=='-'] = refseq$txEnd[refseq$strand=='-'] + DOWN
