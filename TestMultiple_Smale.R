...
targets  = data.frame(num=352298:352302, times=c(0,15,30,60,120))
ChromInfo = cbind(sprintf('chrom_%s/sorted.ssj',targets$num), sprintf('Min_%s',targets$times),sprintf('Min_%s',targets$times))


##########################################################
#### GET SSJ data  (Dmitri D. Pervouchine)
##########################################################
dataDir = sprintf("%s/ExonPipeline/mm10",homedir)
setwd(dataDir)
load("PlottedData_new3.RData")

dataDir = '/home/RNAseq/Smale/mm10/'
splicingData = list()
...
IDs=sub("(.*)_.*","\\1",getNames(sprintf('%s/cyt_%d/polyA.plus.Up',dataDir,targets$num[i]-10)))

refseq2 = subset(refseq, !grepl('random',chrom))
refseq2 = refseq2[!getAllDups(refseq2$name),]

strand = refseq2$strand[match(IDs,refseq2$name)]  

badID = is.na(strand)
strand2 = strand[!badID]
IDs2 = refseq$UniqueID[match(IDs[!badID], refseq$name)]

myCleavedFractions = list()   

## For each of the lists, read in 'Up' and 'Down' reads to determine overlaps:     
for (i in 1:nrow(ChromInfo)){
  Down.Plus = getFileData(sprintf('%s/cyt_%d/polyA.plus.Down',dataDir,targets$num[i]-10))[!badID]
  Down.Minus = getFileData(sprintf('%s/cyt_%d/polyA.minus.Down',dataDir, targets$num[i] -10))[!badID]
  Up.Plus = getFileData(sprintf('%s/cyt_%d/polyA.plus.Up',dataDir,targets$num[i]-10))[!badID]
  Up.Minus = getFileData(sprintf('%s/cyt_%d/polyA.minus.Up',dataDir, targets$num[i] -10))[!badID]
  
  names(Down.Plus) = names(Down.Minus) = names(Up.Plus) = names(Up.Minus) = IDs2
  Up = Up.Plus
  Down = Down.Plus
  Up[strand2=='-'] = Up.Minus[strand2=='-']
  Down[strand2=='-'] = Down.Minus[strand2=='-']
  ...
  myCleavedFractions[[i]] = (AllCleavedFraction2)      
}

names(myCleavedFractions) = ChromInfo[,2] #names(myLists)
...
# sapply(ChromInfo[,2],function(x)c(length(which(!is.na(mergedData2[,x]))),length(which(!is.na(mergedData3[,x])))))
# Min_0 Min_15 Min_30 Min_60 Min_120
# [1,]  6645   7587   5516   5257    5796
# [2,]  6561   7463   5418   5140    5714

# 
#############################################################################
### Analyze Chrom poly(A) RNAseq, 
# and just report the % of genes saved above that have high cleavage levels in this sample
############################################################################

myChrom_CleavedFractions = list()   

## For each of the lists, read in 'Up' and 'Down' reads to determine overlaps:     
for (i in 1:nrow(ChromInfo)){
  Down.Plus = getFileData(sprintf('%s/chrom_%d/polyA.plus.Down',dataDir,targets$num[i]))[!badID]
  Down.Minus = getFileData(sprintf('%s/chrom_%d/polyA.minus.Down',dataDir, targets$num[i]))[!badID]
  Up.Plus = getFileData(sprintf('%s/chrom_%d/polyA.plus.Up',dataDir,targets$num[i]))[!badID]
  Up.Minus = getFileData(sprintf('%s/chrom_%d/polyA.minus.Up',dataDir, targets$num[i]))[!badID]
  ...
  AllCleavedFraction = Up/(Up + Down)#1-Down/Up
  AllCleavedFraction2 = AllCleavedFraction
  AllCleavedFraction2[Up + Down <= 500] <- NA # 500 is equivalent to ~5 reads if length is 100 bp
  myChrom_CleavedFractions[[i]] = (AllCleavedFraction2)      
}

names(myChrom_CleavedFractions) = ChromInfo[,2] #names(myLists)


save(myCleavedFractions,myChrom_CleavedFractions,mergedData2,mergedData3, file='Spring2014_Smale_multiple_timepoints_splicingInfo.RData')
...
for ( i in 1:nrow(ChromInfo)){
   ...
   # First just get the list of genes that I'm considering (b/c they have sufficienct cleavage in the Cyt fraction)
   theseGenes = mergedData3$UniqueID[!is.na(mergedData3[, myColumn])]
   myPolyA[[i]] = myChrom_CleavedFractions[[i]][names(myChrom_CleavedFractions[[i]]) %in% theseGenes]      
}

# Obtain a list of genes that have cleavage Ratio of 0.8 or higher
polyA80 = sapply(1:nrow(ChromInfo),function(i)try(length(which(myPolyA[[i]] > 0.8))/length(myPolyA[[i]][!is.na(myPolyA[[i]])])))
...
for(N in c(100)) {
  ...
  for (i in 1:nrow(ChromInfo)){                                                        
    ...
    print(i)
    starts = c(g=1,k=.001,A=A_max,B=10000)
  ...
  try(rownames(fitCoefs) <- ChromInfo[,2])
  save(QuantileData, SDData, FitData,fitCoefs, file=sprintf('Multiple_Timepoints2_splicing%d.RData',N))
